...
(function (root) {
  ...
  function validate(p) {
    if (!p || !["call", "put"].includes(p.type)) throw new RangeError("期权类型须为 call 或 put。");
    for (const k of ["spot", "strike", "years", "vol", "rate", "dividend"]) {
      if (!Number.isFinite(p[k])) throw new RangeError("所有定价参数必须是有限数值。");
    }
    if (p.spot <= 0 || p.strike <= 0 || p.years <= 0 || p.vol <= 0) throw new RangeError("价格、剩余期限和波动率必须大于零。");
  }
  function quote(p) {
    validate(p);

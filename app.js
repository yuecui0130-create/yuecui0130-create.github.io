(function () {
  ...
  const bounds = {
    spot:[1,10000,"Spot price"], strike:[1,10000,"Strike price"],
    days:[1,3650,"Maturity"], vol:[1,200,"Volatility"],
    rate:[-10,30,"Risk-free rate"], dividend:[0,30,"Dividend yield"]
  };
  ...
  function validateInput(p) {
    if (!p || !["call","put"].includes(p.type)) throw new Error("Select a call or put option.");
    for (const [key,[min,max,label]] of Object.entries(bounds)) {
      if (!Number.isFinite(p[key]) || p[key]<min || p[key]>max || (key==="days" && !Number.isInteger(p[key]))) {
        const err = new Error(label+" must be "+(key==="days"?"an integer ":"a number ")+"between "+min+" and "+max+".");
        err.field = key;
        throw err;
      }
    }
  ...
  function refresh() {
    ...
    try {
      ...
      $("moneyness").textContent = direction===0?"ATM":direction>0?"ITM":"OTM";
      const breakeven = p.strike+(p.type==="call"?quote.price:-quote.price);
      $("breakeven").textContent = breakeven<0?"None for S ≥ 0":fmt(breakeven,2);
      ...
      renderTable();
      clearTimeout(announcement);
      announcement = setTimeout(()=>{$("result-summary").textContent=(p.type==="call"?"Call":"Put")+" option. Model price "+fmt(quote.price)+". Delta "+fmt(quote.delta)+".";},250);
    } catch (e) {
      state = null;
  ...
  const config = {
    value:{title:"Option value vs. spot",legend:"BSM value",unit:"Option value",caption:"All other inputs are held constant. The grey line shows intrinsic value at each spot price."},
    delta:{title:"Delta vs. spot",legend:"Delta",unit:"Delta",caption:"A steeper curve means Delta changes more rapidly with spot. Maturity, volatility, interest rate and dividend yield are held constant."},
    profit:{title:"Long option P&L at expiry",legend:"Net P&L at expiry",unit:"Expiry P&L",caption:"Expiry P&L equals terminal payoff less the current model premium. It excludes fees and financing costs and does not represent mark-to-market P&L before expiry."}
  };
  function setChart(mode,focus=false) {
    if (!config[mode]) throw new Error("Unknown chart measure.");
    chartMode=mode;
    ...
    canvas.setAttribute("aria-label",config[mode].title+". Scenario values are listed in the table below.");
    hover=null;
    $("chart-tooltip").hidden=true;
    drawChart();
  }
  function drawChart() {
    ...
    if (!state) {ctx.fillStyle="#617085";ctx.textAlign="center";ctx.font="14px sans-serif";ctx.fillText("Enter valid inputs to display the chart",w/2,h/2);geometry=null;return;}
    ...
    ctx.textAlign="right";ctx.fillText(chartMode==="profit"?"Spot at expiry":"Spot price",w-right,h-1);
  ...
  if(context && typeof context.registerTool==="function"){
    ...
    const tool={
      name:"configure_european_option",title:"Configure European option pricing",
      ...
      execute(input){if(!input||typeof input!=="object"||Object.keys(input).some(k=>!["type",...Object.keys(bounds)].includes(k)))throw new Error("Invalid input fields.");return configure(input);}
    };
